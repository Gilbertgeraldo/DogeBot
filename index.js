...
const checkRegisteredUser = (sender) => {
    ...
    const ownerNumber = [`6281235136510@s.whatsapp.net`]
...
// Katalog
const reply = (teks) => {
             res = xeon.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 111119999, "message": teks, "footerText": "*GILBERT*", "thumbnail": bgg, "surface": 'CATALOG' }}, {quoted:ftrol})
...
"externalAdReply": {
                "title": `Whatsapp-BOT\n*
                GILBERT BOT. (@gilbert_ajah_hehe) •Instagram photos and Videos`,
                "body": "",
                "previewType": "PHOTO",
...
break
case 'h':
reply('dia sudah pergi bang,semoga dia tenang di alam baka sana. Amin..')
break
case 'okeyx':
reply('HALO MEMBER BARU!!!!.SELAMAT DATANG DI GROUP INI')
break
...
case 'script':
		case 'sc':
		case 'sourcecode':
		xeon.sendMessage(from, { text: "YAHAHAHA WAHYU,NGEMIS SC LU", matchedText: 'nopee', description: "", title: "What are you doing?", jpegThumbnail: iye }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
      //------------------< Sticker Cmd >-------------------
      case "addcmd":
      case "setcmd":
     ...
     break
        ...
        case "broadcast":  
                   if (!isOwner && !mek.key.fromMe) return reply('only owner can use this feature')    
					...
					buff = await xeon.downloadMediaMessage(encmedia)
					for (let _ of anu) {
						xeon.sendMessage(_.jid, buff, image, {quoted:fakestatus ,caption: `「  *GILBERT BOT BROADCAST* 」\n\n${ini_bc}`})
					}
					reply('```Success Broadcast```')
					} else {
					for (let _ of anu) {
					xeon.sendMessage(_.jid, `*GILBERT BOT BROADCAST*\n${ini_bc}`, text, { contextInfo: { mentionedJid: [sender],"forwardingScore":999,"isForwarded":true},sendEphemeral: true })
					}
					reply('```Success Broadcast```')
					}
					break
...
sendButMessage(from, tiyo, `GILBERT BOT`, [
            {              
   ...
   case 'bc':
      case 'broadcast':
        if (!mek.key.fromMe) return;
             ...
             for (let _ of anu) {
             xeon.sendMessage(_.jid, bc, image, {quoted:fkontak,caption: `*「 GILBERT BOT BROADCASTING 」*\n\n${body.slice(4)}`})
}
...
buttons = [{buttonId: `yuri`,buttonText:{displayText: `➡️Next`},type:1}]
              ...
              buttonsMessage = {footerText:'GILBERT BOT', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await xeon.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              xeon.relayWAMessage(prep)
break
